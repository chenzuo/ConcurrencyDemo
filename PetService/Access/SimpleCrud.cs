...
namespace Dapper
{
    ...
    public static class SimpleCRUD
    {
        ...
        public static T Insert<T>(this IDbConnection connection, object entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            var key = GetIdProperties(entityToInsert).FirstOrDefault();

            var newId = connection.Query<T>(sb.ToString(), entityToInsert).Single();
            return (newId == null) ? default(T) : (T)newId;

            //connection.Execute(sb.ToString(), entityToInsert, transaction, commandTimeout);
            //var r = connection.Query(string.Format("select IDENT_CURRENT( '{0}' ) id", name), null, transaction, true, commandTimeout);
            //return (long?)r.First().id;
        }
